...
SLIDES = [
    ...
    # Slide 3: 為什麼需要 CSI？（動機引言）
    {
        ...
        "right_title": "CSI 能做什麼",
        "right_bullets": [
            "每個 OFDM 子載波的複數增益 H(f)",
            "  · 振幅 + 相位，完整通道資訊",
            "",
            "▶ 研究應用",
            "  · 室內定位（ToA / AoA 估測）",
        ...
        "speaker_notes": "在進入技術細節之前，先確立我們為什麼需要 CSI。RSSI 是最傳統的無線量測指標，但它只告訴你總接收功率，無法描述通道的頻率特性。\n\nCSI 則是每個 OFDM 子載波的複數通道增益，包含振幅和相位。這個豐富的資訊讓 CSI 成為室內定位、手勢辨識、呼吸監測等研究的核心工具——這些應用用 RSSI 根本做不到。\n\n今天的核心問題是：研究人員如何在實際硬體上取得 CSI？而 Windows 為什麼做不到？",
    },
    ...
    # Section Break 2: Windows 驅動堆疊
    {
        ...
        "section": 2,
        "title": "Windows 驅動堆疊",
        "subtitle": "從 NIC 硬體向上：逐層解析 CSI 消失的原因",
    },
    # 橋接引言：CSI 要去哪？
    {
        "type": "flow",
        "section": 2,
        "title": "CSI 要去哪？軟體層的問題",
        "flow_items": [
            ("NIC 韌體", "CSI 計算於此\n存放於暫存器"),
            ("Windows\n驅動堆疊", "多層軟體元件\n（本段逐層解析）"),
            ("應用程式", "研究目標\n想在此取得 CSI"),
        ],
        "note": "PHY 工程師熟悉左側；本段目標：找出中間黑盒子裡，CSI 在哪一層消失",
        "speaker_notes": "現在進入 Windows 驅動架構。這是今天的核心問題：CSI 在 NIC 韌體計算完成後，要怎麼到達你的應用程式？\n\n從 RF 工程師的視角來看，左邊是熟悉的領域：PHY 解調、NIC 韌體計算 CSI、儲存在暫存器。右邊是研究目標：你的程式需要 CSI 來做定位或感測。\n\n中間那個「黑盒子」就是 Windows 的驅動堆疊。接下來我們從最靠近硬體的元件開始，逐層往上解析，找出 CSI 究竟在哪裡消失。",
    },
    # Slide 8: OEM.sys 是什麼
    {
        "type": "two_col",
    ...
    # Windows 網路驅動架構總覽（全段回顧）
    {
        ...
        "layers": [
            ("應用程式", "User Mode", ""),
            ("WinSock (ws2_32.dll)", "User Mode", ""),
            ("AFD.sys（Ancillary Function Driver）", "Kernel Mode", ""),
            ("tcpip.sys（TCP/IP Stack）", "Kernel Mode", ""),
            ("ndis.sys（NDIS）", "Kernel Mode", ""),
            ("nwifi.sys（Native WiFi）", "Kernel Mode", ""),
            ("OEM.sys（Miniport Driver）", "Kernel Mode", ""),
            ("NIC 硬體 + 韌體", "Hardware", ""),
        ],
        "note": "每層只能透過明確定義的介面與相鄰層溝通",
        "speaker_notes": "在逐一介紹每個元件之後，讓我們用這張全景圖把所有元件放在一起看。\n\n從最底層的 NIC 硬體開始，往上依序是 OEM.sys（韌體廠商驅動）、nwifi.sys（WiFi 連線管理）、NDIS、tcpip.sys、WinSock，最終到應用程式。\n\n每一層只能透過明確定義的介面與相鄰層溝通。這個設計保證了穩定性，但也意味著 L1 的 PHY 資訊無法「跳過」中間層直接到達應用程式。\n\n接下來我們用這張圖精確標示 CSI 消失的位置。",
    },
    # Slide 14: CSI 消失點分析
    {
    ...
    # Slide 15b: 能不能直接破解 OEM.sys？（可行性評估）
    {
        "type": "two_col",
        "section": 2,
        "title": "能不能直接破解 OEM.sys？",
        "left_title": "理論路徑（四種）",
        "left_bullets": [
            "① 逆向 IOCTL（IDA Pro / Ghidra）",
            "  枚舉 netwtw*.sys dispatch table",
            "",
            "② Kernel Hook，攔截 DMA Buffer",
            "  在 MiniportReceivePackets 插入",
            "",
            "③ 韌體 Patch（類 Nexmon 手法）",
            "  修改 NIC 韌體暴露 CSI",
            "",
            "④ NDIS Filter Driver 夾層",
            "  插入 OEM.sys 與 nwifi.sys 之間",
        ],
        "right_title": "現實障礙（Win11 22H2+）",
        "right_bullets": [
            "⛔ DSE：驅動必須有 WHQL 簽名",
            "⛔ PatchGuard：Kernel patch 觸發 BSOD",
            "⛔ HVCI（Win11 22H2 起預設啟用）",
            "   Hypervisor 層強制 CI，無法繞過",
            "⛔ Intel 韌體閉源：格式未知，無文件",
            "⛔ 更新碎裂：每次 driver update",
            "   IOCTL offset 可能改變",
            "",
            "結論：技術上「可以嘗試」",
            "      工程上「不應嘗試」",
        ...
        "note": "唯一 Windows 可行路徑：PicoScenes + NI USRP（SDR）——但硬體成本遠高於 AX200 + Linux",
        "speaker_notes": "講到這裡，有些聽眾可能會問：「那我們能不能直接破解 OEM.sys，強行讀出 CSI？」\n\n理論上確實有四條路可以嘗試。但每一條在現代 Windows 11 上都撞到了高牆。\n\n最關鍵的障礙是 HVCI——Hypervisor-Protected Code Integrity，從 Windows 11 22H2 開始預設啟用。它在 Hypervisor 層強制執行驅動簽名，不是在 Kernel 層，這意味著即使你嘗試繞過 PatchGuard，HVCI 還是在更底層擋住你。\n\n結論：技術上可以花時間嘗試，工程上完全不應該嘗試。學術界到目前（2026 年）為止沒有任何成功從 Windows 標準 WiFi NIC 提取 CSI 的公開案例。\n\n如果你真的必須在 Windows 環境取得 CSI，唯一的合法路徑是使用 NI USRP 軟體定義無線電搭配 PicoScenes——但硬體成本遠高於 AX200 加 Linux 雙系統。",
    },
